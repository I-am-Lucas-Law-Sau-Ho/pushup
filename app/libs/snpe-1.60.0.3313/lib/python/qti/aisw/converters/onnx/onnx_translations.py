...
class OnnxTranslationBase(translation.ConversionTranslationBase):
    ...
    @staticmethod
    def fetch_constant_op(name, graph, *, prunable=True, quantizable=None, dtype=None, fail_if_dynamic=True,
        ...
        if not graph.has_buffer(name) and graph.weights.has(name):
            ...
            if quantizable is None:
                quantizable = False
                if buf_value.dtype in [np.float32, np.float64]:
                    quantizable = True
...
class OnnxInputTranslation(OnnxTranslationBase):
    ...
    def add_input_op(self, input_, graph, **kwargs):
        ...
        # TODO: switch to commented line once QA updates tests failing when model types are honored.
        return graph.add_input(name, shape)
        # return graph.add_input(name, shape, input_dtype=input_dtype)
