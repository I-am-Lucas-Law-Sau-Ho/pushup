...
class OnnxMatMulTranslation(OnnxTranslationBase):
    ...
    def extract_parameters(self, src_op, graph):
        ...
        if graph.weights.has(self.input_names[0]):
            ...
            tensor = graph.weights.fetch(static_input_name, prunable=False)
            if not graph.has_buffer(static_input_name):
                ops.append(op_adapter.ConstantOp(static_input_name, tensor))

        # If input B is constant, add a Constant Op
        if graph.weights.has(self.input_names[1]):
            weight_input_name = self.input_names[1]
            weights = graph.weights.fetch(weight_input_name, prunable=False)

            # TODO: Remove FC and only use Matmul when SNPE supports QNN completely
            input_A_buf = graph.get_buffer(self.input_names[0])
            if len(weights.shape) == 2:
                # this is to support rank 1 matmul
                if input_A_buf.rank() == 1:
                    if weights.shape[0] == input_A_buf.shape[0]:
                        ops.append(op_adapter.FullyConnectedOp(str(src_op.name), weights, bias))
                        self.input_names.remove(weight_input_name)
                        return ops

                if input_A_buf.rank() == 2:
                    bias = numpy.zeros(weights.shape[-1], dtype=numpy.float32)
                    ops.append(op_adapter.FullyConnectedOp(str(src_op.name), weights, bias))
                    self.input_names.remove(weight_input_name)
                    return ops

                if input_A_buf.rank() == 4:
                    batch, depth, height, width = input_A_buf.shape
                    if weights.shape[0] == width and depth == 1 and height == 1:
                        bias = numpy.zeros(weights.shape[-1], dtype=numpy.float32)
                        self.input_names.remove(weight_input_name)
                        fc_name = self.extract_output_names(src_op, graph)[0] + '_fc'
                        fc_node = graph.add(op_adapter.FullyConnectedOp(fc_name, weights, bias),
                                            self.input_names,
                                            [fc_name])
                        self.add_src_op_info(fc_node.op.name, src_op, graph)
                        reshape_name = self.extract_output_names(src_op, graph)[0] + '_reshape'
                        reshape_node = graph.add(op_adapter.ReshapeOp(reshape_name,
                                                                      [batch, depth, height, weights.shape[-1]]),
                                                 [fc_name],
                                                 self.extract_output_names(src_op, graph),
                                                 axis_formats=[input_A_buf.axis_format]
                                                )
                        self.add_src_op_info(reshape_node.op.name, src_op, graph)
                        return ops

            if not graph.has_buffer(weight_input_name):
                ops.append(op_adapter.ConstantOp(weight_input_name, weights))
            shape_B = weights.shape
        else:
