...
class RelayMirrorPadTranslation(RelayTranslationBase):
    ...
    def translate_op(self,
        ...
        if mode == "SYMMETRIC":
            ir_mode = PadOp.Mode.SYMMETRIC
        elif mode == "REFLECT":
            ir_mode = PadOp.Mode.REFLECT
        else:
            log_assert(False, "Unknown nn.mirror_pad mode: {}", mode)

        ir_op = PadOp(op_name, pads=ir_pad_width, mode=ir_mode)
...
class RelaySpaceToDepthTranslation(RelayTranslationBase):
    ...
    def translate_op(self,
        ...
        log_assert(not attr_dict["mode"] or attr_dict["mode"] == "DCR",
                   "Only support DCR mode, but got {}", attr_dict["mode"])
        log_assert(attr_dict["layout"] == "NHWC",
                   "Only support NHWC layout, but got {}", attr_dict["layout"])

        ir_op = SpaceToDepthOp(op_name, downscale_factor=attr_dict["block_size"])
        return ir_op
