...
from qti.aisw.converters.common.converter_ir.axis_tracker import AxisTracker, AxisOrder, CaffeAxisOrder, OnnxAxisOrder
...
supported_opt_list = [SQUASH_SCALE, SQUASH_PROD, SQUASH_DIV, SQUASH_SUM, SQUASH_SUB, SQUASH_BATCHNORM, FOLD_CONCATS,
                      ...
                      expand_1d_spatial_nn_nodes]

spatial_first_format_to_channel_first_permute_order = {'NSC': AxisTracker.AxisFormat.NSC_TO_NCS,
                                                       'NFC': AxisTracker.AxisFormat.NFC_TO_NCF,
                                                       'NTF': AxisTracker.AxisFormat.NTF_TO_TNF}
spatial_first_format_to_channel_first_format = {'NSC': AxisTracker.AxisFormat.NCS,
...
class IROptimizations(object):
    ...
    def squash_multiple_permute(self, graph):
        ...
        for node_tuple in matched_node_list:
            ...
            first_permute_order = first_permute.op.order
            second_permute_order = second_permute.op.order
            ...
            first_permute.op.order = new_order
...
class OptimizationTranslationBase(translation.Translation):
    ...
    def axes_to_spatial_first_order(self, node: op_graph.OpNode, graph: op_graph.IROpGraph):
        ...
        if input_axis_formats == node.op.data_axis_formats and \
                not AxisTracker.AxisFormat.NONTRIVIAL in input_axis_formats:
...
def squash_node_into_nn_node(graph, matched_node_list):
    ...
    for node_tuple in matched_node_list:
        ...
        if nn_buf.axis_format == AxisTracker.AxisFormat.NCS:
            if nn_op.hasattr("weights") and len(nn_op.weights.shape) == 4:
                ...
                if nn_op.type in [op_adapter.ConvolutionOp.TRANSLATION_KEY,
                    ...
                    nn_op.weights = np.transpose(nn_op.weights, AxisTracker.AxisFormat.OIHW_TO_HWIO)
                elif nn_op.type == op_adapter.DeconvolutionOp.TRANSLATION_KEY:
                    nn_op.weights = np.transpose(nn_op.weights, AxisTracker.AxisFormat.IOHW_TO_HWIO)
        ...
        if node_type in OPS_HAVING_BIAS_SUM:
            ...
            nn_op.bias = np.atleast_1d((nn_op.bias + scale_bias).squeeze())
            ...
            nn_op.bias = np.atleast_1d((nn_op.bias * scale_weights).squeeze())
            ...
            nn_op.weights = nn_op.weights * scale_weights
            nn_op.bias = np.atleast_1d(((nn_op.bias * scale_weights) + scale_bias).squeeze())
        else:
            raise ValueError("Squashing {} node {} into {} node {} unsupported.".format(node_type, node.op.name,
                                                                                        nn_op.type, nn_op.name))

        if nn_op.type == op_adapter.BatchnormOp.TRANSLATION_KEY:
            nn_op.weights = np.atleast_1d(nn_op.weights.squeeze())

        if nn_buf.axis_format == AxisTracker.AxisFormat.NCS:
            if nn_op.hasattr("weights") and len(nn_op.weights.shape) == 4:
                if nn_op.type in [op_adapter.ConvolutionOp.TRANSLATION_KEY,
                                  op_adapter.DepthwiseConvolutionOp.TRANSLATION_KEY]:
                    nn_op.weights = np.transpose(nn_op.weights, AxisTracker.AxisFormat.HWIO_TO_OIHW)
                elif nn_op.type == op_adapter.DeconvolutionOp.TRANSLATION_KEY:
                    nn_op.weights = np.transpose(nn_op.weights, AxisTracker.AxisFormat.HWIO_TO_IOHW)
            if const_op is not None and len(const_op.tensor.shape) == 4:
...
def add_or_broadcast_bias(node, graph, output_depth):
    ...
    if len(node.input_names) < 3:
        bias_tensor = np.zeros([output_depth], dtype=np.float32)
...
class Optimize1DNNTranslation(OptimizationTranslationBase):
    ...
    def add_reshape_op(graph, reshape_op_name, output_shape, input_names, output_names, axis_formats=None, idx=-1):
        node = graph.add(op_adapter.ReshapeOp(reshape_op_name, output_shape=output_shape), input_names, output_names,
    ...
    def reshape_to_2d_spatial(self, input_name, node, graph):
        ...
        output_shape = buffer.shape[:2] + [1] + buffer.shape[2:]
    ...
    def reshape_weights(self, input_name, node, graph):
        ...
        if len(weight_buffer.shape) == 3:
            output_shape = weight_buffer.shape[:2] + [1] + weight_buffer.shape[2:]
            ...
            self.add_reshape_op(graph, weights_reshape_op_name, output_shape,
                                [input_name], [weights_reshape_op_name], axis_formats=[AxisTracker.AxisFormat.OIHW],
    ...
    def reshape_to_1d_spatial(self, nn_2d_output_name, output_names, nn1d_consumers, graph):
        nn_2d_output_buffer = graph.get_buffer(nn_2d_output_name)
        output_shape = nn_2d_output_buffer.shape[:2] + nn_2d_output_buffer.shape[-1:]
    ...
    def expand_1d_spatial_nn_nodes(self, node, graph):
        ...
        self.reshape_to_1d_spatial(nn_2d_output_name, node.output_names, nn1d_consumers, graph)


class OptimizeArgXyzTranslation(OptimizationTranslationBase):
    def __init__(self, op_type):
        super(OptimizeArgXyzTranslation, self).__init__()
...
@register_layer_optimization
class OptimizeArgMaxTranslation(OptimizeArgXyzTranslation):
    def __init__(self):
        OptimizeArgXyzTranslation.__init__(self, op_adapter.ArgMaxOp.TRANSLATION_KEY)


@register_layer_optimization
class OptimizeArgMinTranslation(OptimizeArgXyzTranslation):
    def __init__(self):
        OptimizeArgXyzTranslation.__init__(self, op_adapter.ArgMinOp.TRANSLATION_KEY)
...
class OptimizeBatchnormTranslation(OptimizationTranslationBase):
    ...
    def axes_to_spatial_first_order(self, node, graph):
        input_buf = graph.get_input_buffers(node)[0]
        if input_buf.rank() > 1 and input_buf.rank() <= 4:
            AxisTracker.alter_axis_format_to_ir_order(node, graph)
            ...
            # Enforce the output format here to be NSC/NFC
    ...
    def merge_low_level_ops_to_layers(self, graph):
        ...
        for nodes_tuple in matched_node_list:
            ...
            batchnorm_op.weights = np.atleast_1d(batchnorm_op.weights.squeeze())
            batchnorm_op.bias = np.atleast_1d(batchnorm_op.bias.squeeze())
        ...
        for sequence in sequences:
            for node_tuple in graph.get_matched_nodes(sequence, validator=validate):
                ...
                if conv_node_weights_buffer.axis_format == AxisTracker.AxisFormat.OIHW:
                    weights = np.transpose(conv_node_weights, AxisTracker.AxisFormat.OIHW_TO_HWIO)
                    weights = weights * bn_node.op.weights
                    weights = np.transpose(weights, AxisTracker.AxisFormat.HWIO_TO_OIHW)
                else:
...
class OptimizeConvolutionTranslation(OptimizationTranslationBase):
    ...
    def axes_to_spatial_first_order(self, node, graph):
        if isinstance(graph.src_axis_order, (CaffeAxisOrder, OnnxAxisOrder)):
            ...
            # TODO: Remove NONTRIVIAL from if below. There is no way to know how to handle nontrivial here. But we
            #       need to check for regressions.
            ...
            if any(axis_format in input_axis_formats for axis_format in [AxisTracker.AxisFormat.NSC,
                ...
                # Enforce the output format here to be NSC
...
class OptimizeConvolution1DTranslation(Optimize1DNNTranslation):
    def __init__(self):
        ...
        self.op_type = op_adapter.Convolution1DOp.TRANSLATION_KEY
        self.register_method(expand_1d_spatial_nn_nodes, self.expand_1d_spatial_nn_nodes)

    def expand_1d_spatial_nn_nodes(self, node, graph):
        conv_op_name = node.op.name + "_2d"
        self.nn_2d_op = op_adapter.ConvolutionOp(conv_op_name,
                                                 bias_op_name=node.op.bias_op_name,
                                                 pady_before=node.op.pady_before,
                                                 pady_after=node.op.pady_after,
                                                 padx_before=node.op.padx_before,
                                                 padx_after=node.op.padx_after,
                                                 padding_size_strategy=node.op.padding_size_strategy,
                                                 stridex=node.op.stridey,
                                                 stridey=node.op.stridey,
                                                 dilationx=node.op.dilationy,
                                                 dilationy=node.op.dilationy,
                                                 groups=node.op.groups,
...
class OptimizeConcatTranslation(OptimizationTranslationBase):
    ...
    def axes_to_spatial_first_order(self, node, graph):
        ...
        # TODO: Check if the 'if-check' is necessary
...
class OptimizeCropTranslation(OptimizationTranslationBase):
    ...
    def axes_to_spatial_first_order(self, node, graph):
        ...
        if input_buf.axis_format == AxisTracker.AxisFormat.NSC and \
                node.op.data_axis_formats[0] == AxisTracker.AxisFormat.NCS and \
                (target_buf is None or target_buf.rank() == 4):
            ...
            node.op.counts = AxisTracker.permute_shape(node.op.counts, AxisTracker.AxisFormat.NCS_TO_NSC)
                ...
                node.op.data_axis_formats[0] == AxisTracker.AxisFormat.NCS and \
                (target_buf is None or target_buf.rank() == 3):
            ...
            node.op.offsets = AxisTracker.permute_shape(node.op.offsets, AxisTracker.AxisFormat.TNF_TO_NTF)
            node.op.counts = AxisTracker.permute_shape(node.op.counts, AxisTracker.AxisFormat.TNF_TO_NTF)
...
class OptimizeDeconvolutionTranslation(OptimizationTranslationBase):
    ...
    def axes_to_spatial_first_order(self, node, graph):
        if isinstance(graph.src_axis_order, (CaffeAxisOrder, OnnxAxisOrder)):
            ...
            if any(format in input_axis_formats for format in [AxisTracker.AxisFormat.NSC,
                                                               AxisTracker.AxisFormat.HWIO,
                                                               AxisTracker.AxisFormat.NONTRIVIAL]):
                AxisTracker.image_to_channel_last_order(node, graph)
                output_buffer = graph.get_output_buffers(node)[0]
...
class OptimizeElementwiseTranslation(OptimizationTranslationBase):
    ...
    def axes_to_spatial_first_order(self, node: op_graph.OpNode, graph: op_graph.IROpGraph):
        if len(node.input_names) == 1:
            # Call default method for Unary Op
            super().axes_to_spatial_first_order(node, graph)
        else:
            # Ensure:
            ...
            input_buffers = graph.get_input_buffers(node)
            if matching_ranks(input_buffers):
                ...
                for i, buf in enumerate(input_buffers):
                    if node.op.data_axis_formats[i] != buf.axis_format and \
                            buf.axis_format in AxisOrder().axis_formats:
...
class OptimizeElementwiseDivTranslation(OptimizeElementwiseTranslation):
    def __init__(self):
        OptimizeElementwiseTranslation.__init__(self, op_adapter.ElementwiseDivOp.TRANSLATION_KEY)
...
class OptimizeElementwiseProductTranslation(OptimizeElementwiseTranslation):
    def __init__(self):
        OptimizeElementwiseTranslation.__init__(self, op_adapter.ElementwiseProductOp.TRANSLATION_KEY)
...
class OptimizeElementwiseSubTranslation(OptimizeElementwiseTranslation):
    def __init__(self):
        OptimizeElementwiseTranslation.__init__(self, op_adapter.ElementwiseSubOp.TRANSLATION_KEY)
...
class OptimizeElementwiseSumTranslation(OptimizeElementwiseTranslation):
    def __init__(self):
        OptimizeElementwiseTranslation.__init__(self, op_adapter.ElementwiseSumOp.TRANSLATION_KEY)
...
class OptimizeFullyConnectedTranslation(OptimizationTranslationBase):
    ...
    def squash_batchnorm(graph):
        ...
        for node_tuple in matched_node_list:
            ...
            broadcasted_tensor = np.zeros(len(bn_node.op.weights), dtype=np.float32)
...
class OptimizeL2NormTranslation(OptimizationTranslationBase):
    ...
    def axes_to_spatial_first_order(self, node, graph):
        ...
        input_buf = graph.get_input_buffers(node)[0]
        if input_axis_formats[0] == AxisTracker.AxisFormat.NSC and \
                node.op.data_axis_formats[0] == AxisTracker.AxisFormat.NCS:
            axis_map = graph.src_axis_order.permute_sequence_from_ir[input_buf.rank() - 1]
...
class OptimizeLayerNormTranslation(OptimizationTranslationBase):
    ...
    def match_layer_norm(graph):
        ...
        graph.dump_json("after_layernorm.json")

    def axes_to_spatial_first_order(self, node: op_graph.OpNode, graph: op_graph.IROpGraph):

        input_buffers = graph.get_input_buffers(node)
        if node.op.data_axis_formats[0] == AxisTracker.AxisFormat.NCS and \
            ...
            graph.inject_implicit_permute(input_buffers[0].name,
                                          ...
                                          permute_order,
                                          [node.op.name])
        elif node.op.data_axis_formats[0] == AxisTracker.AxisFormat.NCF and \
            ...
            # No change needed
            return False
        elif input_buffers[0].axis_format == AxisTracker.AxisFormat.NCS or \
...
class OptimizeLstmTranslation(OptimizationTranslationBase):
    ...
    def axes_to_spatial_first_order(self, node, graph):
        ...
        node.op.input_weights = np.ascontiguousarray(node.op.input_weights.transpose(), dtype=np.float32)
        node.op.hidden_state_weights = np.ascontiguousarray(node.op.hidden_state_weights.transpose())

        # LSTM input axis format must be NTF
        input_name = node.input_names[0]
        input_bufs = graph.get_input_buffers(node)
        output_bufs = graph.get_output_buffers(node)
        for i, in_buf in enumerate(input_bufs):
            if in_buf.axis_format == AxisTracker.AxisFormat.NONTRIVIAL or \
                    in_buf.axis_format == AxisTracker.AxisFormat.TNF:
                graph.inject_implicit_permute(node.input_names[i], AxisTracker.AxisFormat.NTF,
    ...
    def preprocess_lstm_ops(graph):
        ...
        for nodes_tuple in matched_node_list:
            ...
            if len(lstm_node.input_names) != 3:
                ...
                initial_cell_state_tensor = np.zeros((batch_size, output_size), dtype=np.float32)
            ...
            # Input weights are expected in [4*hidden_size, input_size]
            # Rec weights are expected in   [4*hidden_size, hidden_size]
            ...
            lstm_node.op.input_weights = np.ascontiguousarray(lstm_node.op.input_weights.transpose(), dtype=numpy.float32)
            lstm_node.op.hidden_state_weights = np.ascontiguousarray(lstm_node.op.hidden_state_weights.transpose())
            lstm_input_split_weights = np.split(lstm_node.op.input_weights, indices_or_sections=4, axis=0)
            lstm_hidden_split_weights = np.split(lstm_node.op.hidden_state_weights, indices_or_sections=4, axis=0)
            gate_split_biases = np.split(lstm_node.op.gate_bias, indices_or_sections=4, axis=0)
            ...
            input_to_forget_w_tensor = np.resize(lstm_input_split_weights[1], (num_units, input_size))
            ...
            input_to_cell_w_tensor = np.resize(lstm_input_split_weights[3], (num_units, input_size))
            ...
            input_to_output_w_tensor = np.resize(lstm_input_split_weights[2], (num_units, input_size))
            ...
            forget_gate_b_tensor = np.resize(gate_split_biases[1], (num_units,))
            ...
            cell_gate_b_tensor = np.resize(gate_split_biases[3], (num_units,))
            ...
            output_gate_b_tensor = np.resize(gate_split_biases[2], (num_units,))
            ...
            input_to_input_w_tensor = np.resize(lstm_input_split_weights[0], (num_units, input_size))
            ...
            if lstm_node.op.cell_weights is not None:
                lstm_cell_split_weights = np.split(lstm_node.op.cell_weights, indices_or_sections=3, axis=1)
            ...
            input_gate_b_tensor = np.resize(gate_split_biases[0], (num_units,))
            ...
            if lstm_node.op.proj_weights is not None:
                ...
                proj_w_tensor = np.resize(lstm_node.op.proj_weights, (output_size, num_units))
            ...
            if lstm_node.op.proj_bias is not None:
                ...
                proj_b_tensor = np.resize(lstm_node.op.proj_bias, (output_size,))
            ...
            if output_reshape_needed:
                ...
                reshape_buffer.axis_format = AxisTracker.AxisFormat.NTF
    ...
    def axes_to_spatial_first_order(self, node, graph):
        ...
        if input_buf.axis_format == AxisTracker.AxisFormat.NSC and \
            ...
            node.op.pads = AxisTracker.permute_shape(node.op.pads, AxisTracker.AxisFormat.NCS_TO_NSC)
                ...
                node.op.data_axis_formats[0] == AxisTracker.AxisFormat.NCF:
            node.op.pads = AxisTracker.permute_shape(node.op.pads, AxisTracker.AxisFormat.NCF_TO_NFC)
        elif input_buf.axis_format == AxisTracker.AxisFormat.NTF and \
                node.op.data_axis_formats[0] == AxisTracker.AxisFormat.TNF:
            node.op.pads = AxisTracker.permute_shape(node.op.pads, AxisTracker.AxisFormat.TNF_TO_NTF)
        node.op.pads = np.asarray(node.op.pads, dtype=np.dtype('int32'))
        AxisTracker.alter_axis_format_to_ir_order(node, graph)
        return True
...
class OptimizePool1DTranslation(Optimize1DNNTranslation):
    ...
    def expand_1d_spatial_nn_nodes(self, node, graph):
        ...
        self.nn_2d_op = op_adapter.PoolOp(pool_op_name,
                                          pool_type=node.op.pool_type,
                                          size_y=node.op.size_y,
                                          size_x=node.op.size_x,
                                          stride_y=node.op.stride_y,
                                          stride_x=node.op.stride_x,
                                          dilation_y=node.op.dilation_y,
                                          dilation_x=node.op.dilation_x,
                                          pady_before=node.op.pady_before,
                                          pady_after=node.op.pady_after,
                                          padx_before=node.op.padx_before,
                                          padx_after=node.op.padx_after,
                                          padding_size_strategy=node.op.padding_size_strategy,
                                          pool_region_include_padding=node.op.pool_region_include_padding)
    ...
    def axes_to_spatial_first_order(self, node, graph):
        ...
        if input_buf.axis_format == AxisTracker.AxisFormat.NSC:
            if node.op.data_axis_formats[0] == AxisTracker.AxisFormat.NSC:
                ...
                if output_buf.axis_format == AxisTracker.AxisFormat.NCS and node.op.order == [0, 3, 1, 2]:
                    graph.replace(node.op,
                                  op_adapter.NoopOp(node.op.name))
                    output_buf.axis_format = AxisTracker.AxisFormat.NSC
                    return
                elif output_buf.axis_format == AxisTracker.AxisFormat.NSC and \
                        node.op.order == [0, 1, 2, 3]:
                    # Nothing to be done, Remove_Noop will handle the squashing of this node
                    return
                else:
                    output_buf.axis_format = AxisTracker.AxisFormat.NONTRIVIAL
                ...
                if output_buf.axis_format == AxisTracker.AxisFormat.NSC and node.op.order == [0, 2, 3, 1]:
                    graph.replace(node.op,
                                  op_adapter.NoopOp(node.op.name))
                    output_buf.axis_format = AxisTracker.AxisFormat.NSC
                    return
            ...
            if node.op.data_axis_formats[0] == AxisTracker.AxisFormat.NFC:
                ...
                if output_buf.axis_format == AxisTracker.AxisFormat.NCF and node.op.order == [0, 2, 1]:
                    graph.replace(node.op,
                                  op_adapter.NoopOp(node.op.name))
                    output_buf.axis_format = AxisTracker.AxisFormat.NFC
                    return
                ...
                if output_buf.axis_format == AxisTracker.AxisFormat.NFC and node.op.order == [0, 2, 1]:
                    graph.replace(node.op,
                                  op_adapter.NoopOp(node.op.name))
                ...
                node.op.data_axis_formats[0] == AxisTracker.AxisFormat.NCF:
            # special case: transforming to NFC, will become noop
            if output_buf.axis_format == AxisTracker.AxisFormat.NFC and node.op.order == [0, 2, 1]:
                node.op.order = [0, 1, 2]
                output_buf.axis_format = AxisTracker.AxisFormat.NFC
                return
            ...
            if node.op.data_axis_formats[0] == AxisTracker.AxisFormat.TNF and node.op.order == [1, 0, 2]:
                node.op.order = [0, 1, 2]
                output_buf.axis_format = AxisTracker.AxisFormat.NTF
            else:
...
class OptimizePreluTranslation(OptimizationTranslationBase):
    ...
    def _permute_coeff(cls, node, graph):
        ...
        # Note: only NSC, NTF formats imply permute was done.
        ...
        if input_permute_order is not None and len(coeff_shape) > 1:
            ...
            node.op.coeff = np.ascontiguousarray(np.transpose(node.op.coeff, input_permute_order))
            coeff_shape = node.op.coeff.shape
        ...
        if input_buf.producer.op.type != op_adapter.PermuteOp.TRANSLATION_KEY:
            if input_buf.axis_format == AxisTracker.AxisFormat.NSC and \
                ...
                graph.inject_implicit_permute(input_name, AxisTracker.AxisFormat.TNF,
                    ...
                    input_buf.axis_format == AxisTracker.AxisFormat.NCF:
                pass
            else:
    ...
    def remove_noop(node, graph):
        ...
        if input_buffer.shape == node.op.output_shape and len(input_buffer.consumers) == 1:
            try:
                ...
                log_debug("Squash Reshape op {} due to Noop not possible ".format(node.op.name))
...
class OptimizeRnnTransformationTranslation(OptimizationTranslationBase):
    ...
    def axes_to_spatial_first_order(self, node, graph):
        AxisTracker.time_series_to_batch_first_order(node, graph)
        return True
...
class OptimizeScatterNDTranslation(OptimizationTranslationBase):
    ...
    def axes_to_spatial_first_order(self, node, graph):
        ...
        if [input_buf.axis_format, indices_buf.axis_format, updates_buf.axis_format] == node.op.data_axis_formats:
            # No change
            return False

        # Check if any of the buffers have been changed into NSC, NFC, NTF order and revert if so
...
class OptimizeSoftmaxTranslation(OptimizationTranslationBase):
    ...
    def axes_to_spatial_first_order(self, node, graph):
        ...
        input_buf = graph.get_buffer(node.input_names[0])
        if input_buf.axis_format == node.op.data_axis_formats[0] and \
                node.op.data_axis_formats[0] in [AxisTracker.AxisFormat.NSC, AxisTracker.AxisFormat.NFC]:
            # No change
            return False
    ...
    def axes_to_spatial_first_order(self, node, graph):
        ...
        for i, input_buf in enumerate(input_bufs):
            ...
            if input_buf.rank() == 3 and input_buf.shape[2] == num_boxes:
                graph.inject_implicit_permute(node.input_names[1], AxisTracker.AxisFormat.NONTRIVIAL,
                                             [0, 2, 1], [node.op.name])
