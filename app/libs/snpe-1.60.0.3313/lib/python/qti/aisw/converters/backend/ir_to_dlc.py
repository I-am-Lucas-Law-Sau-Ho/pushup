...
class DlcConstantTranslation(BackendTranslationBase):
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        node.op.name = node.output_names[0]
        output_nodes = graph.get_op_output_nodes(node)

        # If the constant tensor is input to Eltwise whose other input is Float,
        # mark the constant tensor as float and hence as quantizable
        # This is done to optimize the const -> 32 to 8-bit conversion that the runtime will need to do otherwise
        # to ensure the inputs to eltwise have the same bitwidth.
        if len(output_nodes) ==1:
            consumer = output_nodes[0]
            if isinstance(consumer.op, op_adapter.ElementwiseOp):
                consumer_input_op_types = graph.get_input_op_types(consumer)
                quantizable = any([input_type is not op_adapter.ConstantOp.TRANSLATION_KEY
                                   for input_type
                                   in consumer_input_op_types])
                if node.op.quantizable != quantizable:
                    log_warning("Overriding quantizable for {} to {}".format(node.op.name, quantizable))
                    node.op.quantizable = quantizable
...
class DlcUdoTranslation(BackendTranslationBase):
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        ...
        for i, (name, value) in enumerate(list(node.op.inputs.items())):
            ...
            if value['layout'] == 'NOT_YET_DEFINED' and graph.buffers[name].axis_format == 'NSC':
                node.op.inputs[name]['layout'] = "SNPE_UDO_LAYOUT_NHWC"
            if value['layout'] == 'NOT_YET_DEFINED' and graph.buffers[name].axis_format == 'NCS':
                node.op.inputs[name]['layout'] = "SNPE_UDO_LAYOUT_NCHW"
        ...
        for i, (name, value) in enumerate(list(node.op.outputs.items())):
            ...
            if value['layout'] == 'NOT_YET_DEFINED' and graph.buffers[name].axis_format == 'NSC':
                node.op.outputs[name]['layout'] = "SNPE_UDO_LAYOUT_NHWC"
            if value['layout'] == 'NOT_YET_DEFINED' and graph.buffers[name].axis_format == 'NCS':
                node.op.outputs[name]['layout'] = "SNPE_UDO_LAYOUT_NCHW"
