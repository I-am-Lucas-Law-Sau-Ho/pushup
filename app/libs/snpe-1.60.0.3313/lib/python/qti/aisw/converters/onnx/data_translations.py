...
class OnnxGatherTranslation(OnnxTranslationBase):
    ...
    def extract_parameters(self, src_op, graph):
        ...
        input_names = list(map(str, src_op.input))
        const_input_ops = []
        const_input_op = self.fetch_constant_op(input_data_name, graph, dtype=None, prunable=False, fail_if_dynamic=False)
        if const_input_op is not None:
            const_input_ops.append(const_input_op)
        const_input_op = self.fetch_constant_op(indices_name, graph, dtype=np.int32, prunable=False,
        ...
        if not graph.has_buffer(indices_name) and graph.weights.has(indices_name):
            indices = graph.weights.fetch(indices_name, prunable=False).astype(np.int32)
...
class OnnxPadTranslation(OnnxTranslationBase):
    ...
    def extract_parameters(self, src_op, graph):
        ...
        if pads_name:
            pads_op = self.fetch_constant_op(pads_name, graph, dtype=np.int32)
        ...
        constant_value = 0
        if const_name:
            const_op = self.fetch_constant_op(const_name, graph, dtype=np.int32)
...
class OnnxResizeTranslation(OnnxTranslationBase):
    ...
    def extract_parameters(self, src_op, graph):
        ...
        def get_output_dims_from_scales(scales):
            ...
            # Note: doing round() first since casting to int gets the floor
        ...
        def get_output_dims_from_sizes(sizes):
        ...
        def get_static_tensor(tensor_name, type, graph, dtype=np.float32):
            ...
            """
            tensor = None
            if graph.weights.has(tensor_name):
                tensor = graph.weights.fetch(tensor_name).astype(dtype)
        ...
        if len(src_op.input) == 4:
            ...
            if scales_tensor is None or is_tensor_empty(scales_tensor):
                ...
                output_height, output_width = get_output_dims_from_sizes(sizes)
            else:
                scales = scales_tensor.tolist()
                output_height, output_width = get_output_dims_from_scales(scales_tensor)
            ...
            output_height, output_width = get_output_dims_from_scales(scales)

        scale_height = scales[2]
        scale_width = scales[3]
        # Generate output shape using output_dims.
        ...
        if transformation_mode == "pytorch_half_pixel":
            if output_height > 1 and output_width > 1:
                ...
                # If output height or width == 1, we can only support pytorch_half_pixel for upsampling
                # Since the output height/width is 1, the max scale for upsampling is 1.
                # So there is no resize, input and output height/width are both 1.
                log_assert(scales[-2] == 1,
                           "Unsupported output_height {} and scale {} for pytorch_half_pixel",
                           output_height, scales[-2])
                log_assert(scales[-1] == 1,
                           "Unsupported output_width {} and scale {} for pytorch_half_pixel",
                           output_width, scales[-1])
                pass
...
class OnnxScatterNDTranslation(OnnxTranslationBase):
    ...
    def extract_parameters(self, src_op, graph):
        ...
        const_indices_op = self.fetch_constant_op(indices_name, graph, dtype=np.uint32,
                                                  quantizable=False,
                                                  fail_if_dynamic=False)
        if const_indices_op is not None:
            const_input_ops.append(const_indices_op)

        const_updates_op = self.fetch_constant_op(updates_name, graph, dtype=np.int32,
                                                  quantizable=False,
                                                  fail_if_dynamic=False)
        ...
        if not graph.has_buffer(indices_name) and graph.weights.has(indices_name):
            indices = graph.weights.fetch(indices_name, prunable=False).astype(np.uint32)
...
class OnnxShapeTranslation(OnnxTranslationBase):
    ...
    def extract_parameters(self, src_op, graph):
        ...
        output_name = str(src_op.output[0])
        graph.weights.insert(output_name, np.asarray(shape, dtype=np.int32))
        shape_param = np.asarray(shape, dtype=np.int32)
        ...
        if len(src_op.input) >= 3:
            for key, name in zip(keys[1:], input_names[1:]):
                ...
                if name and graph.weights.has(name):
                    ...
                    params[key] = graph.weights.fetch(name, dtype=np.int64, prunable=False).tolist()
                    if key == 'axes':
...
class OnnxSqueezeTranslation(OnnxTranslationBase):
    ...
    def extract_parameters(self, src_op, graph):
        ...
        if len(src_op.input) > 1:
            ...
            # only support constant for second input, if dynamic fetch will fail.
            axes = self.fetch_constant_op(axes_input, graph, dtype=np.int32).tensor.tolist()
        elif 'axes' in params:
            axes = params.axes
...
class OnnxTileTranslation(OnnxTranslationBase):
    ...
    def extract_parameters(self, src_op, graph):
        ...
        if len(input_names) == 3:
            ...
            # Represents Tile-6
            repeats = graph.weights.fetch(src_op.input[1]).astype(np.uint32).tolist()
        else:
...
class OnnxWhereTranslation(OnnxTranslationBase):
    ...
    def add_op(self, src_op, graph):
        ...
        if op.type == op_adapter.NoopOp.TRANSLATION_KEY:
            self.add_src_op_info(op.name, src_op, graph)
            return graph.add(op, input_names, output_names)
