...
class PermuteLayerBuilder(LayerBuilder):
    def build_layer(self, ir_graph, converter_context, descriptor, input_descriptors, output_descriptors):
        ...
        if order_list == [0, 2, 3, 1]:
            ...
            if ir_graph.get_buffer(input_name).get_axis_format() == AxisTracker.AxisFormat.NTF:
                axis_format = AxisTracker.AxisFormat.TNF
            elif ir_graph.get_buffer(input_name).get_axis_format() == AxisTracker.AxisFormat.TNF:
                axis_format = AxisTracker.AxisFormat.NTF
            else:
