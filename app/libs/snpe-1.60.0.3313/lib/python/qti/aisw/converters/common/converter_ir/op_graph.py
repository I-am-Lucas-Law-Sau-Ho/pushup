...
class OpNode(object):
    ...
    def encode(self, graph):
        ...
        for name in self.input_names:
            input_dict[name] = graph.get_buffer(name).encode(graph)
...
class InputLayout(object):
    ...
    @classmethod
    def get_supported_types(cls):
        return [cls.NCHW, cls.NHWC, cls.NFC, cls.NCF, cls.NTF, cls.TNF, cls.NF, cls.NC, cls.FEATURE, cls.NONTRIVIAL]

    @classmethod
    def get_axis_format(cls, layout):
        layout_to_axis_format_dict = {cls.NHWC: AxisTracker.AxisFormat.NSC,
...
class IROpGraph(object):
    ...
    def add_input(self, name, shape, axis_format=None, input_type=None, input_dtype=None):
        ...
        axis_formats = [axis_format] if axis_format is not None else (
            None if name not in self.input_axis_formats else [self.input_axis_formats[name]])
    ...
    def inject_implicit_permute(self, input_name, target_format, permute_order, consumers:[str] = None):
        ...
        if self.has_buffer(permute_name) and self.get_buffer(permute_name).axis_format == target_format:
            ...
            if isinstance(producer_op, op_adapter.PermuteOp) and producer_op.order == permute_order:
    ...
    def get_input_axis_formats(self, node):
        return [self.buffers[name].axis_format for name in node.input_names]
