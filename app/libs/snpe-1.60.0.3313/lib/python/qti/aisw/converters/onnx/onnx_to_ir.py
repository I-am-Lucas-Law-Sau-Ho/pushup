...
class OnnxConverterFrontend(ConverterFrontend):
    ...
    def convert(self):
        def run_simplifier(model, input_dims: dict = None, skip_optimization=False):
            import onnxsim
            if input_dims:
                ...
                model_optimized, check_ok = onnxsim.simplify(model, perform_optimization=not skip_optimization)
            return model_optimized, check_ok
        ...
        if not self.no_simplification:
            try:
                ...
                check_ok = False
                try:
                    model_optimized, check_ok = run_simplifier(model, input_dims=dims_dict)
                    if check_ok:
                        log_info("Successfully simplified the onnx model!")
                        model = model_optimized
                    else:
                        log_warning("Couldn't simplify the model, attempting simplification by skipping optimization")
                except Exception as e:
                    log_warning("Onnx model simplification failed, trying simplification with skipped optimization.")
                    pass

                if not check_ok:
                    try:
                        model_optimized, check_ok = run_simplifier(model, input_dims=dims_dict, skip_optimization=True)
                        if check_ok:
                            log_info("Successfully simplified the onnx model with skipped optimization!")
                            model = model_optimized
                        else:
                            log_warning("Couldn't simplify the model, attempting normal conversion")
                    except Exception as e:
                        log_warning("Onnx model simplification failed, trying unsimplified model. ({}: {})", type(e),
                                    str(e))
            except ImportError as e:
