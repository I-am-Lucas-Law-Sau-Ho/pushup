...
class Op(ABC):
    ...
    def populate_axis_format(self, graph, buf, src_axis_order, encodings, input_buffers):
        ...
        elif any([in_format == AxisTracker.AxisFormat.NONTRIVIAL for in_format in self.data_axis_formats]):
            buf.set_axis_format(AxisTracker.AxisFormat.NONTRIVIAL)
        elif any([in_format in src_axis_order.axis_formats for in_format in self.data_axis_formats]):
            buf.populate_axis_format(src_axis_order, encodings)
        elif any([in_format in AxisOrders.TF.axis_formats for in_format in self.data_axis_formats]):
            buf.populate_axis_format(AxisOrders.TF, encodings)
        elif any([in_format in AxisOrders.ONNX.axis_formats for in_format in self.data_axis_formats]):
...
class BatchnormOp(Op):
    ...
    def __init__(self, name, weights, bias, **kargs):
        Op.__init__(self, name, self.TRANSLATION_KEY, **kargs)
    ...
    def encode(self):
        attrs = {}
        attrs['weights_shape'] = self.weights.shape
        attrs['bias_shape'] = self.bias.shape
        return attrs
...
class ConcatOp(Op):
    ...
    # TODO: Override populate_axis_format() and add assertion that axis formats of all inputs match
...
class ConvolutionOp(Op):
    ...
    def init_x_dims_params(self, kargs):
    ...
    def __init__(self, name, bias_op_name=None, **kargs):
        ...
        self.init_x_dims_params(kargs)

    def populate_data_axis_formats(self, graph, input_buffers):
        # Override input buffer axis format
        input_buffers[0].axis_format = graph.src_axis_order.get_axis_format(len(input_buffers[0].shape))
        input_buffers[1].axis_format = graph.src_axis_order.conv_weights_format
        super().populate_data_axis_formats(graph, input_buffers)
...
class Convolution1DOp(ConvolutionOp):
    TRANSLATION_KEY = 'convolution1d'

    def init_x_dims_params(self, kargs):
        self.addattr('padx_before', kargs, self.pady_before)
        self.addattr('padx_after', kargs, self.pady_after)
        self.addattr('stridex', kargs, self.stridey)
        self.addattr('dilationx', kargs, self.dilationy)

    def __init__(self, name, bias_op_name=None, **kargs):
        ConvolutionOp.__init__(self, name, bias_op_name, **kargs)

    def infer_shape(self, input_shapes, input_axis_formats, num_outputs, axis_order):
        batch_size, input_height, _ = axis_order.extract_1d_spatial_dims(input_shapes[0])
        filter_size_height, _, output_depth = axis_order.extract_conv_1d_weights_dims(input_shapes[1])
        ...
        output_shape = axis_order.format_1d_spatial_output_shape(batch_size=batch_size,
                                                                 depth=output_depth,
                                                                 height=output_height)
        return [output_shape]

    def set_macs_params(self, input_shapes: list, output_shapes: list, axis_order):
        weights_shape = axis_order.extract_conv_1d_weights_dims(input_shapes[1])
        # = filter size * number of filter positions
        self.macs = np.prod(weights_shape[0:2]) * self.get_general_macs_val(output_shapes)
...
class DeconvolutionOp(Op):
    ...
    def populate_data_axis_formats(self, graph, input_buffers):
        # Override input buffer axis format
        input_buffers[0].axis_format = graph.src_axis_order.get_axis_format(len(input_buffers[0].shape))
        input_buffers[1].axis_format = graph.src_axis_order.deconv_weights_format
        super().populate_data_axis_formats(graph, input_buffers)
...
class ElementwiseOp(Op):
    ...
    def infer_shape(self, input_shapes: list, input_axis_formats, num_outputs: int, axis_order) -> list:
        ...
        if len(input_shapes) == 2:
            ...
            # can be broadcast in IR format along channels without the need to expand and match ranks
...
class LayerNormOp(Op):
    ...
    def encode(self):
        attrs = {'axes': self.axes}
        return attrs

    def infer_shape(self, input_shapes, input_axis_formats, num_outputs, axis_order):
        for i, axis in enumerate(self.axes):
            if axis < 0:
                self.axes[i] += len(input_shapes[0])
        return [input_shapes[0]]
...
class LstmOp(Op):
    ...
    def populate_data_axis_formats(self, graph, input_buffers):
        # Override input buffer axis format
        for in_buf in input_buffers:
            in_buf.axis_format = graph.src_axis_order.get_axis_format(len(in_buf.shape),
    ...
    def set_macs_params(self, input_shapes: list, output_shapes: list, axis_order):
        ...
        # need NTF order to get steps
        _, steps, _ = axis_order.extract_time_series_dims(input_shapes[0])
...
class NonMaxSuppresionOp(Op):
    ...
    def infer_shape(self, input_shapes, input_axis_formats, num_outputs, axis_order):
        ...
        output_shapes = [out_dim_1, out_dim_2, out_dim_3, out_dim_4]

        for i in range(0, len(input_shapes)):
            if i >= 2:
                shape = input_shapes[i][:]
                # determine if [batch, num_boxes, ...] or [num_boxes, ...] for input features to set proper output shape
                # accordingly. For latter case, batch dimension addition is handled in optimization to match spec.
                if input_shapes[0][:2] == input_shapes[i][:2]:
                    shape[1] = self.max_total_detections
                elif input_shapes[0][1] == input_shapes[i][0]:
                    shape[0] = self.max_total_detections
                else:
                    raise ValueError("Unable to determine feature output shape for nms + gather layer {}. "
                                     "Input feature dims must start with either [batch, num_boxes] or [num_boxes] "
                                     "to be processed as nms feature".format(self.name))
...
class PermuteOp(Op):
    ...
    def populate_axis_format(self, graph, buf, src_axis_order, encodings, input_buffers):
        # if permute order is trivial then set output buffer axis format to same as input data
        # otherwise set as per the input data axis format and permute order
        if self.order == [0, 1, 2, 3]:
            buf.set_axis_format(self.data_axis_formats[0])
        elif self.data_axis_formats[0] == AxisTracker.AxisFormat.NSC and self.order == [0, 3, 1, 2]:
            buf.set_axis_format(AxisTracker.AxisFormat.NCS)
        elif self.data_axis_formats[0] == AxisTracker.AxisFormat.NCS and self.order == [0, 2, 3, 1]:
            buf.set_axis_format(AxisTracker.AxisFormat.NSC)
        elif self.data_axis_formats[0] == AxisTracker.AxisFormat.NFC and self.order == [0, 2, 1]:
            buf.set_axis_format(AxisTracker.AxisFormat.NCF)
        elif self.data_axis_formats[0] == AxisTracker.AxisFormat.NCF and self.order == [0, 2, 1]:
            buf.set_axis_format(AxisTracker.AxisFormat.NFC)
        elif self.data_axis_formats[0] == AxisTracker.AxisFormat.NTF and self.order == [1, 0, 2]:
            buf.set_axis_format(AxisTracker.AxisFormat.TNF)
        elif self.data_axis_formats[0] == AxisTracker.AxisFormat.TNF and self.order == [1, 0, 2]:
            buf.set_axis_format(AxisTracker.AxisFormat.NTF)
        elif self.data_axis_formats[0] == AxisTracker.AxisFormat.OIHW and self.order == [2, 3, 1, 0]:
            buf.set_axis_format(AxisTracker.AxisFormat.HWIO)
        elif self.data_axis_formats[0] == AxisTracker.AxisFormat.IOHW and self.order == [2, 3, 0, 1]:
            buf.set_axis_format(AxisTracker.AxisFormat.HWIO)
        elif self.data_axis_formats[0] == AxisTracker.AxisFormat.HWIO and self.order == [2, 3, 0, 1]:
            buf.set_axis_format(AxisTracker.AxisFormat.IOHW)
        elif self.data_axis_formats[0] == AxisTracker.AxisFormat.HWIO and self.order == [3, 2, 0, 1]:
...
class Pool1DOp(PoolOp):
    TRANSLATION_KEY = 'pool1d'

    def init_x_dims_params(self, kargs):
        self.addattr('size_x', kargs, self.size_y)

    def __init__(self, name, **kargs):
        PoolOp.__init__(self, name, **kargs)

    def infer_shape(self, input_shapes, input_axis_formats, num_outputs, axis_order):
        batch_size, input_height, depth = axis_order.extract_1d_spatial_dims(input_shapes[0])
        output_height = self.calc_pool_output_dim(input_height,
                                                  self.size_y,
                                                  self.dilation_y,
                                                  self.pady_before,
...
class ReduceOp(Op):
    ...
    def populate_axis_format(self, graph, buf, src_axis_order, encodings, input_buffers):
        if not self.keep_dims:
            if self.data_axis_formats[0] == AxisTracker.AxisFormat.NSC and self.axes == [1,2]:
                buf.set_axis_format(AxisTracker.AxisFormat.NC)
            elif self.data_axis_formats[0] == AxisTracker.AxisFormat.NCS and self.axes == [2, 3]:
                buf.set_axis_format(AxisTracker.AxisFormat.NC)
            else:
                buf.set_axis_format(AxisTracker.AxisFormat.NONTRIVIAL)
        else:
...
class ReshapeOp(Op):
    ...
    def populate_axis_format(self, graph, buf, src_axis_order, encodings, input_buffers):
        ...
        if self.data_axis_formats[0] == AxisTracker.AxisFormat.NC:
            if len(self.output_shape) == 4 and self.output_shape[2:] == [1,1] and \
                    in_buf.shape[0] == self.output_shape[0]:
                buf.set_axis_format(AxisTracker.AxisFormat.NCS)
            elif len(self.output_shape) == 4 and self.output_shape[1:3] == [1,1] and \
                    in_buf.shape[0] == self.output_shape[0]:
                buf.set_axis_format(AxisTracker.AxisFormat.NSC)
            else:
                buf.set_axis_format(AxisTracker.AxisFormat.NONTRIVIAL)
        elif self.data_axis_formats[0] in [AxisTracker.AxisFormat.NSC, AxisTracker.AxisFormat.NCS] and \
                len(self.output_shape) == 2 and self.output_shape[0] == in_buf.shape[0]:
...
class RoiAlignOp(Op):
    ...
    def infer_shape(self, input_shapes, input_axis_formats, num_outputs, axis_order):
        ...
        if self.tiled_batch_h > 0:
            ...
            output_shape = axis_order.format_spatial_output_shape(batch_size=num_rois,
