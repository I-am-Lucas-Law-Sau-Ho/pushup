...
class OnnxConvTranslation(OnnxTranslationBase):
    ...
    def extract_parameters(self, src_op, graph):
        ...
        if input_buffer.rank() == 3:
            ...
            # Adjust pads, dilations and strides values to align with 2D input
            if len(params.pads) // 2 == 1:
                pads = numpy.repeat(params.pads, 2)
            if len(params.dilations) == 1:
                dilations = numpy.repeat(params.dilations, 2)
            if len(params.strides) == 1:
                strides = numpy.repeat(params.strides, 2)
            params.pads = op_adapter.Convolution1DOp.calc_conv_padding_size([input_shape[2], input_shape[2]],
                                                                             [weights_shape[2], weights_shape[2]],
                                                                             dilations,
                                                                             strides,
                                                                             padding_mode,
                                                                             pads)
            # slice back 1D pads
            params.pads = params.pads[0:2]
            ...
            num_output_channels = graph.src_axis_order.extract_conv_1d_weights_dims(weights_shape)[-1]
            convolution_class = op_adapter.Convolution1DOp
            if params.group == num_input_channels and num_input_channels == num_output_channels:
                raise ValueError("Depthwise 1D convolution operation is not yet implemented")
            ...
            params.pads = op_adapter.ConvolutionOp.calc_conv_padding_size(input_shape[2:],
                                                                          ...
                                                                          params.pads)
            # Handle marking this Convolution as a DepthwiseConvolution
            num_input_channels = graph.src_axis_order.extract_spatial_dims(input_shape)[-1]
