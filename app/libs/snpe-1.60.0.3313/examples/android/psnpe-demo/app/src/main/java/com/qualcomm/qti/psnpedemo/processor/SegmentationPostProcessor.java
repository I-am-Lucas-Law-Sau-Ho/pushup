...
public class SegmentationPostProcessor extends PostProcessor {
    ...
    public boolean postProcessResult(ArrayList<File> bulkImage) {
        ...
        for(int i=0; i<imageNum; i++)
        {
            ...
            float[] batchOutput = outputMap.get(outputNames[0]);
            results.add(batchOutput);
        }
        calculateResults(bulkImage,groundTruthPath, results, imageNum);
        return false;
    }

    public void calculateResults(ArrayList<File> bulkImage, String annoFolder, List<float []> results, int imageNum) {
        ...
        for(int imageIndex = 0; imageIndex < imageNum; imageIndex++)
        {
            ...
            String bulkImagePath = (bulkImage.get(imageIndex)).getPath();
